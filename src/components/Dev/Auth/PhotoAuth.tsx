...
import bg from '@/assets/auth/bg_auth2.webp'
              ...
              <div className="w-[320px] min-h-[250px] bg-[#ffffff] flex flex-col rounded-[5px] px-5 pt-7 pb-4 gap-y-4">
                ...
                <div className='flex flex-col gap-y-3'>
                  <div className='relative w-full h-[50px] border border-blue-500 rounded-[5px] px-2 overflow-hidden'>
                    <label htmlFor='email' className={`absolute ${labelAnimationEmail || fieldValueEmail.length > 0 ? 'top-1 pl-2 text-[12px] text-blue-400' : 'top-3 pl-2 text-[16px] text-gray-600'} font-light transition-all duration-200`}>Email</label>
                    <input id='email' value={fieldValueEmail} onChange={({ target }) => { setFieldValueEmail(target.value) }} onMouseDown={() => { setLabelAnimationEmail(true) }} onBlur={() => { setLabelAnimationEmail(false) }} type="email" className='absolute font-light text-gray-600 bg-transparent bottom-1 px-2 w-full h-5 ring-0 focus:ring-0 focus:outline-none' />
                  </div>
                  <div className={`relative w-full ${currStep === 2 ? 'h-[50px]' : 'h-0 hidden'} border border-blue-500 rounded-[5px] px-2 overflow-hidden transition-all duration-200`}>
                    <label htmlFor='password' className={`absolute ${labelAnimationPassword || fieldValuePassword.length > 0 ? 'top-1 pl-2 text-[12px] text-blue-400' : 'top-3 pl-2 text-[16px] text-gray-600'} font-light transition-all duration-200`}>Password</label>
                    <input id='password' value={fieldValuePassword} onChange={({ target }) => { setFieldValuePassword(target.value) }} onMouseDown={() => { setLabelAnimationPassword(true) }} onBlur={() => { setLabelAnimationPassword(false) }} type="password" className='absolute font-light text-gray-600 bg-transparent bottom-1 px-2 w-full h-5 ring-0 focus:ring-0 focus:outline-none' />
                  </div>
                  {
                    currStep === 1 ? (
                ...
                <div className="flex justify-center gap-x-1">
                  ...
                  <button className='text-[14px] text-blue-500 font-light'>Sign up here.</button>
                </div>
              </div>
