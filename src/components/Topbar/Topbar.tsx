  ...
  return (
    <>
      <motion.div
        variants={fadeIn(
          "top",
          pathname.includes("contact") || pathname.length < 2 ? 2 : 0.8,
        )}
        initial={"hidden"}
        whileInView={"show"}
      ...
      >
        <Link to={"/"} className="flex justify-start items-center">
          <img
            alt="logo"
            src={
              "https://res.cloudinary.com/maulight/image/upload/v1781589517/r4lwyz3optp9ivxfu1cq.webp"
            }
            className="w-9 h-9"
          />
        </Link>
        <WideMenu
          setUpperMenuOpen={setUpperMenuOpen}
          isUpperMenuOpen={isUpperMenuOpen}
        />
        <div className="hidden max-sm:flex justify-end items-center">
          <Hamburger
            toggled={isOpen}
            toggle={setOpen}
            color={pathname.includes("video") ? "#e7eee7" : "#231c1e"}
          />
        </div>
      </motion.div>
      {isUpperMenuOpen && (
      ...
      {isUpperMenuOpen && (
        <button
          onClick={() => {
            setUpperMenuOpen(false);
          }}
          className="fixed top-0 left-0 w-screen h-screen z-30"
        ></button>
      )}
      {isOpen && (
        <WideMenuOptionsVertical
          setUpperMenuOpen={setOpen}
